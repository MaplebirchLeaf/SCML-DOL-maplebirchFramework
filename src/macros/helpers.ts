...
export type MacroArgs = unknown[] & { full?: string; raw?: string };
...
export interface MacroContext extends Omit<SugarCubeMacroContext, 'args' | 'createShadowWrapper' | 'error' | 'payload'> {
  args: MacroArgs;
...
export function readStyle(args: unknown[], start = 1): StyleArgs {
  let className = '';
  let style = '';
  let icon = '';
  let iconOnly = false;
  ...
  for (let i = start; i < args.length; i++) {
    ...
    if (arg && typeof arg === 'object' && !Array.isArray(arg)) {
      if ('class' in arg && typeof arg.class === 'string') className = arg.class;
      if ('style' in arg && typeof arg.style === 'string') style = arg.style;
      if ('icon' in arg && typeof arg.icon === 'string') icon = arg.icon;
      if ('iconOnly' in arg && (arg.iconOnly === true || arg.iconOnly === 'true')) iconOnly = true;
      continue;
    }
