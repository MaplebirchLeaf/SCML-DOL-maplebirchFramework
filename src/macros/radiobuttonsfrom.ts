...
function parseRadio(input: unknown): unknown[] | null {
...
export function _radiobuttonsfrom(this: MacroContext): HTMLElement | void {
  ...
  if (typeof varPath !== 'string' || !/^[$_]\S+/.test(varPath)) return this.error(`Variable '${varPath}' needs a sigil.`);
  ...
  const options: Array<{ value: string; data: unknown; label: JQuery }> = [];
    ...
    options.forEach((option, index) => {
      try {
        ...
        if (Array.isArray(data)) {
          ...
          displayText = text((data as Record<string, unknown>)[language] ?? (data as Record<string, unknown>).EN ?? (data as Record<string, unknown>).CN ?? option.value);
        } else {
