// ./src/compat/SimpleFrameworks.ts
...
class SimpleFrameworks {
  ...
  public static addto(zoneName: string, ...widgets: any[]): void {
    maplebirch.tool.addTo(SimpleFrameworks.zoneMap[zoneName] || zoneName, ...widgets);
  }
  ...
  public static readonly TimeEvent = class TimeEvent {
    ...
    private register(): void {
      const success = maplebirch.dynamic.regTimeEvent(this.type, this.eventId, {
        ...this.options,
        cond: (timeData: any) => !!this._cond(SimpleFrameworks.compatTimeData(timeData)),
        action: (timeData: any) => this._action(SimpleFrameworks.compatTimeData(timeData))
      });
...
(window as any).simpleFrameworks = { addto: SimpleFrameworks.addto };
(window as any).TimeEvent = SimpleFrameworks.TimeEvent;
