...
function hairgradients(): HairGradientsReturn {
	...
	for (const [style, hairstyles] of Object.entries(hg.fringe || {})) if ((hairstyles as HairStyleData).all?.colors) data.fringe[style] = (hairstyles as HairStyleData).all!.colors.map(color => color[0]);
...
class Variables {
	...
	Init() {
		try {
			if (this.tool.core.passage?.title === 'Start2') { V.maplebirch = clone({ ...defaultVar, version: this.version }); return; }
...
(async function(maplebirch: any) {
	'use strict';
	await maplebirch.register('var', Object.seal(new Variables(maplebirch)), ['tool']);
})(maplebirch);
