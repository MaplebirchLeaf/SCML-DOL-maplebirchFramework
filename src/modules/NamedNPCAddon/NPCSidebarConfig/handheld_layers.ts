...
const handheld_layers = {
  ...
  nnpc_handheld_left: {
    ...
    srcfn(options: LayerOptions) {
      ...
      const category =
        directory === 'props'
          ? ((handheld.type as string[]).find((type: string) => ['food', 'ingredient', 'recipe', 'tending', 'antique', 'sex toy', 'child toy', 'book', 'furniture'].includes(type)) || 'general') + '/'
          : '';
  ...
  nnpc_handheld_left_acc: {
    ...
    showfn(options: LayerOptions) {
      const nnpc = options.maplebirch.nnpc;
      return (
        nnpc.clothes!.handheld!.index > 0 && nnpc.clothes!.handheld!.leftImage === 1 && nnpc.clothes!.handheld!.accessory === 1 && nnpc.arm_left !== 'none' && !nnpc.hide_all && nnpc.show && nnpc.model
      );
    },
    zfn(options: LayerOptions) {
      return options.maplebirch.nnpc.arm_left === 'cover' ? maplebirch.char.ZIndices.hands + options.maplebirch.nnpc.position! : options.maplebirch.nnpc.zarms! + 0.2;
