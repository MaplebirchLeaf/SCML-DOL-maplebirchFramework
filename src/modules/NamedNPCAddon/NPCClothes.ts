...
class NPCSidebarData {
  ...
  async import(modName: string, modZip: ModZipReader, filePaths: string | string[]) {
    ...
    for (const filePath of paths) {
      ...
      else { maplebirch.npc.log(`不支持的文件格式: ${filePath}`, 'WARN'); continue; }
      ...
      dataArray.forEach((item: any, i: number) => {
        ...
        this.configs.push({
          key, name: npcName, body: item.body ?? '',
...
class WardrobeClothing {
  ...
  register(location: string, key: string, cond?: any) {
    ...
    list.push({ key, cond });
    this.outfits.includes(key) || this.outfits.push(key);
  }
  ...
  get worn(): WardrobeItem {
    ...
    return NPCWardrobe.mergeWithNaked(
      NPCWardrobe.wardrobe[
        find(this.location[loc] || []) ??
        find(this.global) ??
        'naked'
      ]
    );
...
class NPCWardrobe {
  ...
  static register(npcName: string, location: string, key: string, cond?: any) {
    ...
    cloth.outfits.includes(key) || cloth.outfits.push(key);
  ...
  static worn(npcName: string) {
    ...
    if (!cloth) { maplebirch.npc.log(`没有对应NPC:${npcName}数据`, 'WARN'); return; }
    return cloth.worn;
  }
}
...
const NPCClothes = ((core: typeof maplebirch) => {
  ...
  const init = async (manager: NPCManager) => {
    try { await Promise.all([NPCWardrobe.init(manager), vc.init()]); }
    catch (e: any) { core.npc.log(`NPC服装初始化失败: ${e.message}`, 'ERROR'); }
  };
