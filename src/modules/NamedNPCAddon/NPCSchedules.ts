...
class Schedule {
	daily: string[] = new Array(24).fill('');
	...
	at(scheduleConfig: ScheduleTime, location: string): this {
		if (Array.isArray(scheduleConfig) && scheduleConfig.length === 2) {
			...
			if (start > (end ?? start)) { maplebirch.npc.log('起始时间不能大于结束时间', 'ERROR'); return this; }
			_.range(start, (end ?? start) + 1).forEach(hour => { if (hour >= 0 && hour <= 23) this.daily[hour] = location; });
	...
	update(specialId: string | number, updates: Partial<Omit<SpecialSchedule, 'id'>>): this {
		...
		if (special) { _.assign(special, updates); this.sortedSpecials = null; }
		return this;
	}
	...
	topologicalSort(items: SpecialSchedule[]) {
		...
		const queue = _.chain(Array.from(inDegree.entries())).filter(([_, degree]) => degree === 0).map(([id]) => id).value();
	...
	createEnhancedDate(date: DateTime): EnhancedDate {
		...
		for (const key in date) {
			if (!Object.prototype.hasOwnProperty.call(enhancedDate, key)) {
				Object.defineProperty(enhancedDate, key, { 
					get: function() { return (date as any)[key]; } 
				});
			}
		}
		...
		for (const key in Time) {
			if (typeof (Time as any)[key] !== 'function' && !Object.prototype.hasOwnProperty.call(enhancedDate, key)) {
				Object.defineProperty(enhancedDate, key, { 
					get: function() { return (Time as any)[key]; } 
				});
			}
		}
...
const NPCSchedules = ((core: typeof maplebirch) => {
	function initData(manager: NPCManager) {
		if (!_.isArray(manager.NPCNameList)) { manager.log('NPCSchedules: 需要传入NPC名称数组', 'WARN'); return false; }
		for (const npcName of manager.NPCNameList) if (!schedules.has(npcName)) schedules.set(npcName, new Schedule());
		return true;
	}

	function addData(npcName: string, scheduleConfig: ScheduleTime | ScheduleCondition, location: string | ScheduleLocation, id?: string | number, options: Partial<Omit<SpecialSchedule, 'condition' | 'location'>> = {}) {
