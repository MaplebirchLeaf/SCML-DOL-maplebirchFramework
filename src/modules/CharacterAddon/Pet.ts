...
export interface PetSettings extends PetOptions {
  enabled?: boolean;
}
...
interface FloatingPetConfig {
  ...
  storageKey: string;
  className: string;
  fallback(size: number): { left: number; top: number };
}
...
export abstract class FloatingPet {
  protected canvas?: HTMLCanvasElement;
  protected model?: CanvasModel;
  protected container?: HTMLElement;
  protected options: Required<PetOptions> = { ...DEFAULT_OPTIONS };
  ...
  public unmount(): void {
    ...
    const box = this.container ?? document.getElementById(this.petConfig.elementId);
  ...
  public refresh(): boolean {
    if (!this.canvas || !this.model) return false;
  ...
  protected mount(container: HTMLElement, model: CanvasModel, canvas: HTMLCanvasElement): void {
    ...
    const maskSrc = this.host.mask(this.options.mask, this.options.rotation);
    container.classList.add(this.petConfig.className);
    ...
    Object.assign(canvas.style, { width: `${size}px`, height: `${size}px`, transform: '', imageRendering: 'auto' });
  ...
  private clearBox(box: HTMLElement): void {
    box.replaceChildren();
    box.classList.remove(this.petConfig.className, 'dragging');
    box.removeAttribute('style');
  }

  private enableDrag(box: HTMLElement): void {
    const size = this.displaySize;
    const clamp = (left: number, top: number) => ({ left: Math.clamp(left, 0, Math.max(0, window.innerWidth - size)), top: Math.clamp(top, 0, Math.max(0, window.innerHeight - size)) });
    ...
    const savePosition = (): void => {
      ...
      localStorage.setItem(this.petConfig.storageKey, JSON.stringify(clamp(rect.left, rect.top)));
    };
    const position = loadPosition();
    const fallback = this.petConfig.fallback(size);
...
class Pet extends FloatingPet {
  ...
  public constructor(private manager: Character) {
    super(manager, {
      elementId: PET.elementId,
      storageKey: PET.storageKey,
      className: 'maplebirch-character-pet',
      fallback: size => ({ left: window.innerWidth - size - DEFAULT_OFFSET.right, top: window.innerHeight - size - DEFAULT_OFFSET.bottom })
    });
  }
  ...
  public render(target: PetTarget, options: PetOptions = {}): boolean {
    ...
    try {
      ...
      this.mount(container, model, canvas);
