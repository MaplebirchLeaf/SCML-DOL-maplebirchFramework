...
const layers: Record<string, LayerConfig> = {
	...
	left_iris: {
		srcfn: faceStyleSrcFn(
			(options: { trauma: any; eyes_half: any; }) => {
  ...
  constructor(readonly core: MaplebirchCore) {
    ...
    this.mask = mask;
		...
		this.core.once(':sugarcube', () => {
			...
			Object.defineProperty(model, 'layers', {
				get: () => merge(originalLayers, this.layers, { 
					mode: 'merge', 
	...
	process(type: 'pre' | 'post', options: any) {
		...
		for (const fn of handlers) {
			try { fn(options); }
			catch (e: any) { this.log(`${type}process 错误: ${e.message}`, 'ERROR'); }
		}
	}
...
(async function(maplebirch) {
	'use strict';
	await maplebirch.register('char', Object.seal(new Character(maplebirch)), ['var']);
})(maplebirch)
