...
export class Save {
  ...
  public use<Result>(variables: Record<string, unknown>, callback: () => Result): Result {
    ...
    try {
      ...
      if (result != null && (typeof result === 'object' || typeof result === 'function') && 'then' in result && typeof result.then === 'function') {
        void Promise.resolve(result).catch(error => console.error('Save.use rejected an asynchronous callback:', error));
...
export class SugarCube {
  ...
  public save(saveObj: unknown, details?: unknown): Save {
    return new Save(this.require().State, saveObj, details);
  }
}
