...
- Check protected mods through `auth.json` during first load.
...
The framework can verify protected mods that depend on `maplebirch`. Mods without `auth.json` are allowed normally. Mods with `auth.json` ask the player for an author-provided credential on first load, then remember the resolved password.

Place `auth.json` at the protected mod root:
...
```json
{
  ...
  "prompt": {
    "title": "Mod Encryption",
    "label": "Cool Mod Credential",
    "placeholder": "Enter credential",
    "hint": "Enter the credential provided by the author."
  }
}
```
...
| :---------- | :------- | :----------------------------------------------------------- |
| `key`       | Yes      | Credential identifier                                        |
| `publicKey` | Yes      | Public key for signature verification, as JWK or SPKI base64 |
| `subject`   | No       | Credential subject, defaults to the mod name                 |
| `name`      | No       | Mod name shown in the dialog                                 |
| `prompt`    | No       | Dialog text such as `title`, `label`, `placeholder`, `hint`  |
| `date`      | No       | Date credential options such as `timezone`, `graceDays`      |

To use a custom auth file path or a guard generated by author tools, add `auth` to `maplebirchAddon.params`:
...
If `guard` fails, the framework disables the mod. Key generation, credential generation, and `.zip` to `.modpack` conversion are available in [dol-mod-protection-tools](https://github.com/MaplebirchLeaf/dol-mod-protection-tools).
