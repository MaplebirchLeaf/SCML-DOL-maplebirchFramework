...
_可通过 `maplebirch.tool.patch.tips.add()` 访问。_

### 添加小贴士

```javascript
maplebirch.tool.patch.tips.add('general', '第一条模组小贴士。', '第二条模组小贴士。');
```
...
框架会在原版 `init_tips` 完成后调用 `apply()`，因此不会覆盖或阻止原版小贴士初始化。通过 `maplebirchAddon` 加载的普通 `script` 可以直接调用 `add()`。
