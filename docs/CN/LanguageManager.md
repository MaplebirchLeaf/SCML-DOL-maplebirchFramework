...
每种语言分别记录当前实例中各模组首次导入该语言的顺序，同名键由后导入的模组优先提供。重复导入某个模组的同一语言只更新内容，不会改变优先级；文件命中缓存时也遵循相同规则。不同语言的导入顺序互不影响，重载后各语言按各自的首次导入顺序重新确定优先级；尚未重新导入的语言保留原有缓存。

每个模组、每种语言的导入内容是一份完整快照。删除文件中的键或导入空对象 `{}` 会移除该来源的对应翻译；若其它来源仍提供同名键，则恢复优先级最高的剩余翻译，不影响其它语言或模组的词条。
...
- 旧数据库无需清空，正常导入时会补建各来源的缓存。旧格式未保存被覆盖的原文，因此这些来源需要重新导入文件后才能用于回退。
