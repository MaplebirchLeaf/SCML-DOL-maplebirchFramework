...
Use `maplebirch.tool.patch.bodywriting.delete(key)` to remove an entry. Registered operations are applied during initialization; call `maplebirch.tool.patch.bodywriting.apply()` to apply them manually. Removal also clears the index mapping.
