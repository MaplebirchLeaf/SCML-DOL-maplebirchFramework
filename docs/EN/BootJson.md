...
Vanilla categories continue to follow vanilla content settings. New categories are always enabled by default and automatically join the pool built by vanilla `generateTipsList`. Scripts may also call `maplebirch.tool.patch.tips.addTips('myMod', 'A new tip')`. The framework merges tips after vanilla `init_tips` and ignores duplicate text.
