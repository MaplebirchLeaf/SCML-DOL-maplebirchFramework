...
- 修复 `NPC侧边栏` 人模等错误
- 删除战斗模块的 **`战斗反应`** _（原版已可以直接实现）_、**`战斗射精`** _（原版已可以直接实现）_、`异装对话`
- 添加 **`食物注册`** _（0.5.9.x 以上版本可用）_ 以及 **`古董注册`** 功能
- **`游戏版本 0.5.9.7`** 简化 **`简易框架`** 的兼容维护 _（保留 addto 以及 TimeEvent）_
- 优化 `模块管理` 的注册以及使用方式
- 修复 **`时间事件`** 在 `游戏版本 0.5.9.7` 的错误
- 优化 `NPC日程` 的注册逻辑
- 在模组设置中添加 `重置框架默认设置` 的按钮
- 提供了 **`模组加密`** 相关功能。
- 扩展并优化 **`音频模块`** 相关功能。
- 新增可发布的 **`@scml-maplebirch/types`** 类型包，补充 `maplebirch`、`SugarCube` 与全局类型声明，便于插件项目引用。
- 修复并优化 **`类型包`** 的生成、入口导出与声明内容，减少类型引用异常。
- 重构 **`音频模块`** 的内部结构，拆分 `Track`、`Playlist`、`AudioBufferPlayer`，并同步更新中英文音频文档。
- 修复 **`TimeEvent`** 兼容接口与时间事件触发逻辑的相关问题。
