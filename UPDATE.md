...
- 修复 `char.use(layers, 'combatMainPc')` 在战斗主画布中可能只影响新建模型、无法补到 `animateCombat` 缓存实例的问题。
- 优化 `CanvasModel` 图层注入逻辑，缓存模型缺少新增图层时会复用原版构造流程初始化缺失层，减少对原版渲染层默认值的污染。
- 优化 **桌宠** 渲染与同步逻辑，减少重复克隆与渲染压力。
- 简化 **PC转化** 图层注册逻辑，当前转化注册默认服务于 `main` 模型，避免不同画布之间的图层规则混用。
- 同步更新转化相关文档与类型包内容。
